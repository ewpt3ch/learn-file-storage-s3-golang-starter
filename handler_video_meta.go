...
func (cfg *apiConfig) handlerVideoGet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "failed signed url", err)
		return
	}
...
func (cfg *apiConfig) handlerVideosRetrieve(w http.ResponseWriter, r *http.Request) {
	...
	for i, video := range videos {
		respVid, err := cfg.dbVideoToSignedVideo(video)
		if err != nil {
			respondWithError(w, http.StatusInternalServerError, "failed sign url", err)
			return
		}
		videos[i] = respVid
	}
